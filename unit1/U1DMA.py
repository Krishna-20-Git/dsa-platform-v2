    ...
    <body>
        ...
        <script>
            ...
            function drawMemory(blocks) {
                ...
                const scale = 2;  // pixels per memory unit
                const blockSpacing = 10;
                let totalWidth = 50; // Start padding
                for (let block of blocks) {
                    totalWidth += (block.size * scale) + blockSpacing;
                }
                totalWidth += 50; // End padding
                
                // FIX: Dynamically resize canvas
                canvas.width = Math.max(1100, totalWidth);
                ...
                ctx.fillStyle = "#000";
                ctx.textAlign = "left";
                ctx.fillText("Memory Start (0)", 50, 160);

                for (let block of blocks) {
                    let width = block.size * scale;
                    let displayWidth = Math.max(1, width); 

                    ctx.beginPath();
                    ctx.rect(x, y, displayWidth, 80);
                    ...
                    ctx.font = "11px Arial";
                    ctx.fillText(`${block.start}`, x, y + 95);
                    
                    let endLabel = `${block.end}`;
                    let endLabelWidth = ctx.measureText(endLabel).width;
                    let endLabelX = x + displayWidth - endLabelWidth;
                    
                    if (endLabelX < x + 15) { 
                        endLabelX = x + 15;
                    }
                    ctx.fillText(endLabel, endLabelX, y + 95);

                    x += displayWidth + blockSpacing;
                }
                ...
                if (blocks.length > 0) {
                    ...
                    ctx.font = "14px Arial";
                    ctx.textAlign = "left";
                    ctx.fillText(`Memory End (${blocks[blocks.length-1].end})`, x - 30, 160);
                }
            }
