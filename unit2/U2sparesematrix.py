...
# 2. Create Blueprint
sparesematrix_bp = Blueprint('sparesematrix_bp', __name__)
...
# 3. Change routes to use the blueprint
@sparesematrix_bp.route('/')
def index():
    ...
    <head>
        ...
        <style>
            ...
            /* FIX: Added canvas wrapper for scrolling */
            .canvas-wrapper {
                width: 90%%;
                max-width: 1200px;
                overflow-x: auto;
    ...
    <body>
        ...
        <script>
            ...
            function drawMatrix(elements) {
                ...
                // FIX: Dynamically resize canvas width
                const nodeDrawWidth = 120; // (90 width + 30 gap)
                let requiredWidth = 100 + (elements.length * nodeDrawWidth) + 100;
                ...
                for (let i = 0; i < elements.length; i++) {
                    ...
                    nodeX += nodeDrawWidth; // Use fixed spacing
                }
...
# FIX: Changed from @app.route and added
@sparesematrix_bp.route('/status')
def status():
